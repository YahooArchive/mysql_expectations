...
Feature: have_engine_type matcher

  This matcher tests if a table has the expected engine type
  ...
  Scenario: engine type matches
    ...
    And the description should be "should have engine type 'InnoDB'"

  Scenario: engine type does not match
    Given the expectation:
      """ruby
      # Note that the engine_type is incorrect
      #
      it { is_expected.to have_engine_type('MyISAM') }
      """
    When I run rspec
    Then the exit status should be 1
    And the description should be "should have engine type 'MyISAM'"
    And the failure message should be "expected 'item' table engine type to be 'MyISAM' but it was 'InnoDB'"
